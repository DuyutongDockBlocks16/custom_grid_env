...
class MiniGridEnvCustom(gym.Env):
    ...
    def __init__(
        ...
        self.other_agents_pos: dict[int, np.ndarray | tuple[int, int] | None] = {}
        self.other_agents_dir: dict[int, int | None] = {}
        
        self.other_agents = other_agents
        
        self.number_of_other_agents = len(other_agents)

        for i in range(self.number_of_other_agents):
            self.other_agents_pos[i] = None
            self.other_agents_dir[i] = None
    ...
    # other_agents = {
    #     "agent_1": {"start_pos": (1, 2), "start_dir": 0},
    # }
    def place_other_agents(self):
        for i in range(self.number_of_other_agents):
            agent_info = self.other_agents[i]
            start_pos = agent_info.get("start_pos", None)
            start_dir = agent_info.get("start_dir", 0)

            if start_pos is not None:
                self.other_agents_pos[i] = start_pos
                # pos = self.place_obj(None, top=None, size=None, max_tries=math.inf)
                self.other_agents_dir[i] = start_dir
    ...
    def step(
        ...
        self._command_agent(action)
        
        self._command_other_agents()
    ...
    def _command_agent(self, action):
        ...
        # Move forward
            ...
            if fwd_cell is None or fwd_cell.can_overlap():
                ...
                if self.carrying:
                    self.move_obj(self.carrying, self.agent_pos[0], self.agent_pos[1])
        ...
        # Drop an object
            ...
            if self.carrying:
                self.grid.set(agent_pos[0], agent_pos[1], self.carrying)
                self.carrying = None
    ...
    def _command_other_agents(self):
        pass
